...
|------|----------------|--------------------|
| `--frontmost-window` | Title of the frontmost window | **Accessibility** (System Preferences → Privacy & Security → Accessibility). Required to query window names via System Events. |
| `--clipboard` | Clipboard text and types | Pasteboard access (often no prompt; may vary). |
| `--apps` | List of running apps (name, bundle_id, pid) | Uses System Events; same as default frontmost. |
| `--calendar` | Next calendar events | **Calendar** (Privacy & Security → Calendars). |
| `--reminders` | Incomplete reminders | **Reminders** (Privacy & Security → Reminders). |
| `--battery` | Battery % and charging | No special permission. |
| `--network` | Primary interface, SSID, local reachability | No special permission. |
